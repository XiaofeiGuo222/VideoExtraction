...
namespace face
{
	...
	private:
		...
		double alarm(double * psBuff, int n)
		{
			...
			//start from a non-negative num
			if (n - sli_width + 1 >= 0)				        
			{
				//for (int i = 0; i < n - sli_width + 1; i++)
				//{
				//	//g(p) = -p
				//	//f(p) =  g(p - 1/2) = -(p - 1/2) = -p + 1/2
				//	sum += -psBuff[i] + 1 / 2;
				...
				{
					//double psbuff = -psBuff[i] + 1 / 2;
					sum += -psBuff[i] + 1 / 2;
					//std::cout << "i = " << i << "s max = " << max << " sum = " << sum << " buff = " << psbuff << "\n";
/*
					std::cout << "sum = " << fabs(sum) << " max = " << fabs(max) << "\n";*/
			...
			double temp = sqrt((double)sli_width / ((double)12 * significance_level));

			//std::cout << "s max = " << max << " n = " << n << " sig = " << temp << "\n";


			if (fabs(max) >= sqrt((double)sli_width / ((double)12 * significance_level)))
			{
